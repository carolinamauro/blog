      ...
      <div className="mx-auto max-w-4xl">
        <div className="animate-fade-in-up mb-12">
          ...
          <p className="mt-3 max-w-2xl text-zinc-300">
            {t("Cosas que estoy construyendo, explorando o que ya terminé. Hacé click en cualquiera para ver el detalle completo.", lang)}
          </p>
        </div>
        ...
        <div className="grid gap-5">
          {projects.map((project) => {
            ...
            return (
              <Link
                key={project.slug}
                href={localizedHref(`/projects/${project.slug}`, lang)}
              ...
              >
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="flex-1">
                    ...
                    <p className="mt-1 text-sm text-zinc-400">{t(project.tagline, lang)}</p>
                  </div>
                  <div className="min-w-[100px] sm:text-right">
                    <p className="text-sm font-semibold text-zinc-300">{project.progress}%</p>
                    <p className="text-xs text-zinc-500">{t("completado", lang)}</p>
                  </div>
                </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  {project.stack.length > 5 && (
                    <span className="rounded-md border border-white/10 bg-white/[0.05] px-2.5 py-1 text-xs font-medium text-zinc-500">
                      +{project.stack.length - 5} {t("más", lang)}
                    </span>
                  )}
                </div>
